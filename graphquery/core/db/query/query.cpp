...
graphquery::database::query::CQueryEngine::interaction_complex_2(const uint64_t _person_id, [[maybe_unused]] uint64_t _max_date) const noexcept
{
    ...
    {
        auto message_props = get_graph()->get_properties_by_vertex_id_map(edge.src);
        ...
        auto friend_props               = get_graph()->get_properties_by_vertex_id_map(edge.dst);
...
graphquery::database::query::CQueryEngine::interation_complex_8(const uint64_t _person_id) const noexcept
{
    ...
    std::vector<storage::ILPGModel::SEdge_t> person_comments =
        get_graph()->get_edges("Message", "HAS_CREATOR", [&_person_id](const auto & edge) -> bool { return edge.dst == _person_id; });
    ...
    {
        auto comment_props = get_graph()->get_properties_by_vertex_id_map(edge.src);
...
void
graphquery::database::query::CQueryEngine::interation_short_2(const uint64_t _person_id) const noexcept
{
...
void
graphquery::database::query::CQueryEngine::interation_short_7(const uint64_t _message_id) const noexcept
{
