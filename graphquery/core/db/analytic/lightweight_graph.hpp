...
namespace graphquery::database::analytic
{
    ...
    inline CLightWeightGraphModel::CLightWeightGraphModel(const std::vector<storage::ILPGModel::SEdge_t> & edges): IModel()
    {
        const auto remapped = remap(edges);
        build_graph(remapped);
    }

    inline std::vector<storage::ILPGModel::SEdge_t> CLightWeightGraphModel::remap(const std::vector<storage::ILPGModel::SEdge_t> & edges) noexcept
    {
    ...
    inline void CLightWeightGraphModel::build_graph(const std::vector<storage::ILPGModel::SEdge_t> & edges) noexcept
    {
        m_graph.resize(n);
        indegree.resize(n);
    ...
    inline int64_t CLightWeightGraphModel::get_num_edges()
    {
        return num_edges;
    }

    inline int64_t CLightWeightGraphModel::get_num_vertices()
    {
        return n;
    }

    inline uint32_t CLightWeightGraphModel::out_degree(const storage::Id_t id) noexcept
    {
        return m_graph[id].size();
    }

    inline uint32_t CLightWeightGraphModel::in_degree(const storage::Id_t id) noexcept
    {
        return indegree[id];
    }

    inline void CLightWeightGraphModel::calc_outdegree(uint32_t out[]) noexcept
    {
#pragma omp parallel for default(none) shared(out)
    ...
    inline void CLightWeightGraphModel::calc_indegree(uint32_t in[]) noexcept
    {
#pragma omp parallel for default(none) shared(in)
    ...
    inline void CLightWeightGraphModel::calc_vertex_sparse_map(storage::Id_t sparse[]) noexcept
    {
#pragma omp parallel for default(none) shared(sparse)
    ...
    inline int64_t CLightWeightGraphModel::get_total_num_vertices() noexcept
    {
        return n;
    }

    inline void CLightWeightGraphModel::edgemap(const std::unique_ptr<IRelax> & relax)
    {
#pragma omp parallel for default(none) shared(relax)
    ...
    inline void CLightWeightGraphModel::src_edgemap(const storage::Id_t _src, const std::function<void(int64_t src, int64_t dst)> & func)
    {
        for (const auto dst : m_graph[_src])
            func(_src, dst);
    }

    inline std::unique_ptr<std::vector<std::vector<int64_t>>> CLightWeightGraphModel::make_inverse_graph() noexcept
    {
