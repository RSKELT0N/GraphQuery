...
graphquery::database::storage::CDBStorage::init(std::filesystem::path path)
{
	...
	std::string file_name = fmt::format("{}.gdb", path.filename());

	if (!CDiskDriver::check_if_file_exists(path.generic_string()))
		set_up(file_name);
	else
		load(file_name);
}
...
void
graphquery::database::storage::CDBStorage::set_up(std::filesystem::path db_name)
{
	_log_system->info(fmt::format("Initialising new database file: {}", db_name.generic_string()));

	CDiskDriver::create_folder(m_db_disk.get_path() / db_name.replace_extension(""));
	m_db_disk.set_path(m_db_disk.get_path() / db_name.replace_extension(""));

	CDiskDriver::create_file(m_db_disk.get_path() / db_name, MASTER_DB_FILE_SIZE);
	m_db_disk.open(db_name.generic_string());
...
void
graphquery::database::storage::CDBStorage::load(std::filesystem::path db_name)
{
	m_db_disk.open(db_name.generic_string());
	load_db_superblock();
	load_db_graph_table();
	...
	_log_system->info(
		fmt::format("Database file ({}) has been loaded into memory", db_name.generic_string()));
															  ...
															  const std::string & type) noexcept
{
	...
	{
		m_graph_model_lib =
			std::make_unique<dylib>(dylib(fmt::format("{}/{}", PROJECT_ROOT, "lib/models"), type));
		m_graph_model_lib->get_function<void(std::unique_ptr<IMemoryModel> &)>("create_graph_model")(
			m_loaded_graph);
		m_loaded_graph->init(m_db_disk.get_path().parent_path().generic_string(), name);
	}
