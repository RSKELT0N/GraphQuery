...
graphquery::database::storage::CDiskDriver::SRet_t
graphquery::database::storage::CDiskDriver::create_file(const std::filesystem::path & file_path,
...
graphquery::database::storage::CDiskDriver::SRet_t
graphquery::database::storage::CDiskDriver::create_folder(const std::filesystem::path & folder_path)
{
	if (check_if_folder_exists(folder_path.generic_string()))
	{
		m_log_system->warning("Cannot create folder that already exists");
		return SRet_t::ERROR;
	}

	if (mkdir(folder_path.generic_string().c_str(), 0777))
		return SRet_t::ERROR;
