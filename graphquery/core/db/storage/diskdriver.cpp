...
graphquery::database::storage::CDiskDriver::Create(std::string_view file_path, size_t file_size)
{
    ...
    {
        graphquery::database::_log_system->Error(fmt::format("File descriptor ({}) could not be created to create file", file_path));
        return SRet_t::ERROR;
    }

    if(ftruncate(this->m_file_descriptor, file_size) == -1)
    {
        graphquery::database::_log_system->Warning(fmt::format("Issue truncating the file to the specified size {}", errno));
...
graphquery::database::storage::CDiskDriver::Open(std::string_view file_path, int file_mode, int map_mode_prot, int map_mode_flags)
{
    ...
    if(this->m_file_descriptor == -1)
    {
        graphquery::database::_log_system->Error(fmt::format("File descriptor ({}) could not be created to open file", errno));
        return SRet_t::ERROR;
    }

    if(fstat(this->m_file_descriptor, &this->m_fd_info) == -1)
    {
        graphquery::database::_log_system->Error(fmt::format("Issue getting file descriptor ({}) info", file_path));
        return SRet_t::ERROR;
    }
    ...
    if (this->m_memory_mapped_file == MAP_FAILED) {
        graphquery::database::_log_system->Error(fmt::format("Error mapping file to memory"));
        return SRet_t::ERROR;
    }
...
graphquery::database::storage::CDiskDriver::Close()
{
    if(this->m_initialised)
    {
        if (munmap(this->m_memory_mapped_file, this->m_fd_info.st_size) == -1) {
            graphquery::database::_log_system->Error(fmt::format("Error unmapping file from memory"));
            return SRet_t::ERROR;
        }
    } else graphquery::database::_log_system->Warning("File has not been initialised");
...
graphquery::database::storage::CDiskDriver::Read(void *ptr, size_t size, uint32_t amt) const
{
    if(this->m_initialised)
    {
        memcpy(ptr, (const void *)&this->m_memory_mapped_file[this->m_seek_offset], size * amt);
    } else graphquery::database::_log_system->Warning("File has not been initialised");
    return SRet_t::VALID;
}
...
graphquery::database::storage::CDiskDriver::Write(const void *ptr, size_t size, uint32_t amt)
{
    if(this->m_initialised)
    {
        memcpy((void *)&this->m_memory_mapped_file[this->m_seek_offset], ptr, size * amt);
    } else graphquery::database::_log_system->Warning("File has not been initialised");
    return SRet_t::VALID;
}
