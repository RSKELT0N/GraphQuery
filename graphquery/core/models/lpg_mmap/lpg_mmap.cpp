...
CMemoryModelMMAPLPG(const std::shared_ptr<logger::CLogSystem> & log_system, const bool & sync_state_): ILPGModel(log_system, sync_state_), m_master_file(LPG_MAP_MODE)
{
...
graphquery::database::storage::CMemoryModelMMAPLPG::create_graph(std::filesystem::path path, const std::string_view graph) noexcept
{
    ...
    m_edge_offset_ptr_file.store_metadata();
}
...
graphquery::database::storage::CMemoryModelMMAPLPG::setup_files(const std::filesystem::path & path, const bool initialise) noexcept
{
    ...
    m_edge_offset_ptr_file.open(path, EDGE_OFF_PTRS_FILE_NAME, initialise);
}
...
graphquery::database::storage::CMemoryModelMMAPLPG::store_vertex_entry(const Id_t id, const std::unordered_set<uint16_t> & label_ids, const std::vector<SProperty_t> & props) noexcept
{
    ...
    data_block_ptr->payload.edge_off_ptr_idx      = END_INDEX;
...
graphquery::database::storage::CMemoryModelMMAPLPG::store_edge_off_ptr_entry(const uint32_t next_ref, const uint16_t edge_label_id) noexcept
{
    ...
    data_block_ptr->payload.edge_label_id = edge_label_id;

    return entry_offset;
}

void
graphquery::database::storage::CMemoryModelMMAPLPG::store_edge_entry(const Id_t src,
                                                                     const Id_t dst,
                                                                     const uint16_t edge_label_id,
                                                                     const std::vector<SProperty_t> & props,
                                                                     bool undirected) noexcept
{
...
{
    auto v_ptr = m_vertices_file.read_entry(src_idx);

    std::optional<uint16_t> edge_off_idx = get_edge_offset_ptr(v_ptr->payload.edge_off_ptr_idx, edge_label_id);

    if (!edge_off_idx.has_value())
        return false;

    auto edge_off_ptr = m_edge_offset_ptr_file.read_entry(*edge_off_idx);
    auto v_edge_idx   = edge_off_ptr->payload.edge_idx;
    ...
    while (v_edge_idx != END_INDEX)
    {
        ...
        for (uint8_t i = 0; i != e_curr->payload_amt;)
        {
            if (e_curr->state.test(i))
            {
                if (e_curr->payload[i].metadata.dst == dst_idx && e_curr->payload[i].metadata.edge_label_id == edge_label_id)
                    return true;
                i++;
            }
        }
                                                                   ...
                                                                   const bool undirected) noexcept
{
    ...
    store_edge_entry(src_idx, dst_idx, edge_label_id, props, false);
    ...
    if (undirected)
    {
        ...
        store_edge_entry(dst_idx, src_idx, edge_label_id, props, true);
    }
...
graphquery::database::storage::CMemoryModelMMAPLPG::get_edge(const Id_t src_vertex_id, const std::string_view edge_label, const int64_t dst_vertex_id)
{
    ...
    auto v_ptr = get_vertex_by_id(src_vertex_id);

    if (!v_ptr.has_value())
        return std::nullopt;

    auto pred  = [dst_vertex_id](const SEdge_t & edge) -> bool { return dst_vertex_id == edge.dst; };
    auto edges = get_edges_by_offset(src_vertex_id, edge_label_id.value(), pred);
...
graphquery::database::storage::CMemoryModelMMAPLPG::get_edges(const std::string_view vertex_label, const std::string_view edge_label, const std::function<bool(const SEdge_t &)> & pred)
{
    ...
    std::vector<SEdge_t> ret;
    ret.reserve(labelled_vertices_size);
...
#pragma omp parallel for schedule(static) reduction(insert : ret)
    for (size_t i = 0; i < labelled_vertices_size; i++)
    {
        ...
        auto edge_off_idx = get_edge_offset_ptr(vertex_ptr->payload.edge_off_ptr_idx, label_id);

        if (!edge_off_idx.has_value())
            continue;

        auto off_curr_edge_ptr = gbl_edge_off_ptr + *edge_off_idx;
        uint32_t curr          = off_curr_edge_ptr->payload.edge_idx;

        while (curr != END_INDEX)
        {
            auto curr_edge_ptr = gbl_e_ptr + curr;

            for (size_t j = 0; j != curr_edge_ptr->payload_amt;)
            {
                if (likely(curr_edge_ptr->state.test(j)))
                {
                    if (pred(curr_edge_ptr->payload[j].metadata))
...
graphquery::database::storage::CMemoryModelMMAPLPG::get_edges_by_offset(const uint32_t src_vertex_id, const uint32_t dst_vertex_id)
{
    ...
    while (curr_off != END_INDEX)
    {
        ...
        curr_off = curr_off_edge_ptr->next;
    }
...
{
    auto src_vertex_ptr      = get_vertex_by_offset(vertex_id);
    std::vector<SEdge_t> ret = {};
    ...
    ret.reserve(utils::atomic_load(&src_vertex_ptr.value()->payload.metadata.outdegree));
    std::optional<uint32_t> edge_off_ref = get_edge_offset_ptr(src_vertex_ptr.value()->payload.edge_off_ptr_idx, edge_label_id);

    if (!edge_off_ref.has_value())
        return ret;
...
graphquery::database::storage::CMemoryModelMMAPLPG::src_edgemap(const Id_t vertex_offset, const std::function<void(int64_t src, int64_t dst)> & relax)
{
    ...
    auto edge_off_ref     = v_ptr->payload.edge_off_ptr_idx;
    auto gbl_edge_ptr     = m_edges_file.read_entry(0).ref;
    auto gbl_edge_off_ptr = m_edge_offset_ptr_file.read_entry(0).ref;

    while (edge_off_ref != END_INDEX)
    {
        auto e_off_ptr = gbl_edge_off_ptr + edge_off_ref;
        auto edge_head = e_off_ptr->payload.edge_idx;
...
graphquery::database::storage::CMemoryModelMMAPLPG::persist_graph_changes() noexcept
{
    ...
    for (Id_t i = 0; i < datablock_c; i++)
    {
        ...
        auto edge_off_ref = src_vertex_ptr->payload.edge_off_ptr_idx;

        while (edge_off_ref != END_INDEX)
        {
            auto edge_off_ptr      = m_edge_offset_ptr_file.read_entry(edge_off_ref);
            uint32_t edge_ref      = edge_off_ptr->payload.edge_idx;
            ...
            while (edge_ref != END_INDEX)
            {
                ...
                if (edge_ptr->payload_amt == 0)
                {
                    ...
                    if (edge_ref == prev_edge_ref)
                        edge_off_ptr->payload.edge_idx = END_INDEX;
                }

                prev_edge_ref = edge_ref;
                edge_ref      = edge_ptr->next;
            }
            edge_off_ref = edge_off_ptr->next;
        }
    }
}
...
graphquery::database::storage::CMemoryModelMMAPLPG::get_edges_by_label(const std::string_view label)
{
    ...
    ret.reserve(read_edge_label_entry(edge_label_id)->item_c);

    auto gbl_edge_off_ptr = m_edge_offset_ptr_file.read_entry(0);
    auto gbl_edge_ptr     = m_edges_file.read_entry(0);
    auto edge_off_block_c = m_edge_offset_ptr_file.read_metadata()->data_block_c;

    for (uint32_t i = 0; i < edge_off_block_c; i++)
    {
        auto lcl_edge_off_ptr = gbl_edge_off_ptr + i;

        if (lcl_edge_off_ptr->payload.edge_label_id != edge_label_id)
            continue;

        auto edge_ref = lcl_edge_off_ptr->payload.edge_idx;
        while (edge_ref != END_INDEX)
        {
            auto lcl_edge_ptr = gbl_edge_ptr + edge_ref;

            for (size_t j = 0; j < lcl_edge_ptr->state.size(); j++)
            {
                if (lcl_edge_ptr->state.test(j))
                    ret.push_back(lcl_edge_ptr->payload[j].metadata);
            }
            edge_ref = lcl_edge_ptr->next;
        }
    }
    return ret;
}
...
std::unordered_set<graphquery::database::storage::Id_t>
graphquery::database::storage::CMemoryModelMMAPLPG::get_edge_dst_vertices(const Id_t src, const uint16_t edge_label_id, const std::function<bool(const SEdge_t &)> & pred)
{
    std::unordered_set<Id_t> ret        = {};
    SRef_t<SVertexDataBlock> vertex_ptr = {};

    if (const auto v_exists = get_vertex_by_id(src); unlikely(!v_exists.has_value()))
        return ret;
    else
    ...
    std::optional<uint32_t> edge_off_ref = get_edge_offset_ptr(vertex_ptr->payload.edge_off_ptr_idx, edge_label_id);
    ...
    while (curr != END_INDEX)
    {
        ...
        for (size_t i = 0; i < lcl_edge_ptr->payload_amt;)
        {
            if(lcl_edge_ptr->state.test(i))
            {
...
graphquery::database::storage::CMemoryModelMMAPLPG::get_edge_dst_vertices(const Id_t src, const std::string_view edge_label, const std::string_view vertex_label)
{
    ...
    return get_edge_dst_vertices(src,
                                 edge_label_id,
                                 [&gbl_label_ref_ptr, &gbl_v_ptr, &vertex_label_id](const SEdge_t & edge) -> bool
                                 {
                                     const auto dst_vertex_ptr = gbl_v_ptr + edge.dst;
                                     auto label_head           = dst_vertex_ptr->payload.metadata.label_id;
...
{
    bool found                                 = false;
    uint32_t head                              = edge_off_ptr_idx;
    SRef_t<SEdgeOffsetPtrDataBlock> gbl_eo_ptr = m_edge_offset_ptr_file.read_entry(0);

    while (head != END_INDEX && !found)
    {
        const auto lcl_eo_ptr = gbl_eo_ptr + head;
        if (lcl_eo_ptr->payload.edge_label_id == edge_label_id)
            found = true;

        if (!found)
            head = lcl_eo_ptr->next;
    }
...
graphquery::database::storage::CMemoryModelMMAPLPG::rm_edge_entry(const Id_t src, const Id_t dst) noexcept
{
    ...
    auto pred          = [&edge_c, dst_idx, &dst_vertex_ptr](SRef_t<SEdgeDataBlock> & edge_block_ptr) -> void
    {
        for (uint8_t i = 0; i < edge_block_ptr->payload_amt;)
        {
    ...
    {
        auto edge_off_ptr = m_edge_offset_ptr_file.read_entry(head_edge_off_idx);
        edge_c += m_edges_file.foreach_block(edge_off_ptr->payload.edge_idx, pred);
        head_edge_off_idx = edge_off_ptr->next;
    }
...
graphquery::database::storage::CMemoryModelMMAPLPG::rm_edge_entry(const Id_t src, const Id_t dst, const std::string_view edge_label) noexcept
{
    ...
    uint32_t head_edge_off_idx;

    if (const std::optional<uint32_t> opt_head_edge_off_idx = get_edge_offset_ptr(src_vertex_ptr.value()->payload.edge_off_ptr_idx, *edge_label_id); !opt_head_edge_off_idx.has_value())
        return EActionState_t::invalid;
    else
        head_edge_off_idx = *opt_head_edge_off_idx;

    auto edge_off_ptr   = m_edge_offset_ptr_file.read_entry(head_edge_off_idx);
    m_edges_file.foreach_block(edge_off_ptr->payload.edge_idx, pred);
