...
namespace graphquery::database::storage
{
    class CMemoryModelMMAPLPG final : public ILPGModel
    {
      ...
      public:
        ...
        [[nodiscard]] std::unordered_set<int64_t> get_edge_dst_vertices(const SNodeID & src, std::string_view edge_label, std::string_view vertex_label) override;
        ...
        [[nodiscard]] std::unordered_set<int64_t> get_edge_dst_vertices(const SNodeID & src, const std::function<bool(const SEdge_t &)> & pred) override;
      ...
      private:
        ...
        void store_index_entry(int64_t id, const std::unordered_set<uint16_t> & label_ids, uint32_t vertex_offset) noexcept;
        uint32_t store_vertex_entry(int64_t id, const std::unordered_set<uint16_t> & label_id, const std::vector<SProperty_t> & props, uint32_t next_ref) noexcept;
        uint32_t store_edge_entry(uint32_t next_ref, int64_t src, int64_t dst, uint16_t edge_label_id, const std::vector<SProperty_t> & props) noexcept;
        ...
        [[nodiscard]] std::vector<SEdge_t> get_edges_by_id(int64_t src, uint16_t src_label_id, const std::function<bool(const SEdge_t &)> & pred);
